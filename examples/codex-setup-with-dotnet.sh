...
sudo apt-get update
sudo apt-get install -y curl ca-certificates wget apt-transport-https software-properties-common python3

echo "Installing .NET SDK if needed..."
...
# Install the restricted GitHub proxy client.
# This script intentionally does not install or authenticate the real GitHub CLI.
bash <(cat <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
echo "Checking GitHub repository target..."

if [ -z "${GH_REPO:-}" ]; then
  if [ -n "${GITHUB_REPOSITORY:-}" ]; then
    export GH_REPO="$GITHUB_REPOSITORY"
    echo "GH_REPO is set from GITHUB_REPOSITORY: $GH_REPO"
  else
    echo "GH_REPO/GITHUB_REPOSITORY is not set. gh commands must use explicit --repo OWNER/REPO."
  fi
else
  echo "GH_REPO is set: $GH_REPO"
fi
...
if [ -n "${GH_REPO:-}" ]; then
  if git remote get-url origin >/dev/null 2>&1; then
    ...
    echo "Setting git remote origin from GH_REPO..."
    git remote add origin "https://github.com/${GH_REPO}.git"
  fi
  git remote -v
else
  echo "Skipping git remote setup because GH_REPO is not set."
fi

if [ -f "BirthTodo.sln" ]; then
  echo "Restoring solution..."
  dotnet restore BirthTodo.sln

  echo "Building solution..."
  dotnet build BirthTodo.sln --configuration Release --no-restore
else
  echo "BirthTodo.sln not found yet. Skipping dotnet restore/build."
fi
